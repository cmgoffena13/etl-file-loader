...
class S3fsFileWrapper:
    ...
    def _log_progress(self, bytes_read: int):
        ...
        if current_mb >= self._last_logged_mb + 4:
            logger.debug(
                f"Downloaded Total: {current_mb:.2f} MB from S3 for file: {self._filename}"
            )
        ...
        if not self._closed:
            if self._bytes_downloaded > 0:
                ...
                if total_mb >= 0.01:  # Show MB if >= 1KB
                    logger.info(
                        f"Finished downloading {total_mb:.2f} MB from S3 for file: {self._filename}"
                    )
                else:
                    total_kb = self._bytes_downloaded / 1024
                    logger.info(
                        f"Finished downloading {total_kb:.2f} KB from S3 for file: {self._filename}"
                    )
            else:
                logger.info(
                    f"Finished downloading 0.00 MB from S3 for file: {self._filename}"
                )
        self._closed = True
